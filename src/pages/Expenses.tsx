...
const Expenses = () => {
  ...
  // Expense creation/editing modal state
  ...
  const loadExpenses = async () => {
    setLoading(true);
    try {
      const [expensesData, vendors, projectsData] = await Promise.all([
      ...
      const projectLookup = projectsData.reduce<Record<number, string>>((acc, project) => {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          toast.error("PDF not found. Please edit the expense to regenerate the PDF.", {
            action: {
              label: "Edit Expense",
              onClick: () => openEditExpenseModal(expenseId),
            },
          });
        } else {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setAllExpenses(prev => prev.filter(exp => exp.id !== expenseToDelete));
    ...
    <AppShell>
      <div className="space-y-4 sm:space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <Button className="w-full sm:w-auto" onClick={openCreateExpenseModal}>
