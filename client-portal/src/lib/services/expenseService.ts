...
export const expenseService = {
  ...
  async getExpensesByProjectId(projectId: number): Promise<Expense[]> {
    try {
      const response = await apiClient.get<ExpensesResponse>(`/api/expenses/project/${encodeId(projectId)}`);
      return response.expenses || [];
    } catch (error: any) {
